...
ERR="$1"
echo "Watching for line with words: \"eth1deposits\" and $ERR"
shift
"$@" |& while IFS= read -r line; do
    echo "$line"
    if [[ "$line" == *"eth1deposits"* && "$line" == *"$ERR"* ]]; then
        echo "Found line with words \"eth1deposits\" and \"$ERR\". Exiting the program..."
        exit 0
    fi
    if [[ "$line" == *"error"* ]]; then
